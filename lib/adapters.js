...
(function (root) {
  ...
  const arcDateLit = (src, ms) => {
    if (!src.fields.whenIsDateOnly) return "timestamp '" + sqlTs(arcIn(src, ms)) + "'";
    const p = tzParts(ms, src.timezone);
    return "date '" + p.year + '-' + p.month + '-' + p.day + "'";
  };
  const arcWhere = (src, from, to, group, tod) =>
    src.fields.when + ' >= ' + arcDateLit(src, from) + ' AND ' + src.fields.when + ' <= ' + arcDateLit(src, to + (src.fields.whenIsDateOnly ? 0 : 60000)) +
  ...
  function arcRow(src, f) {
    ...
    if (fl.whenIsDateOnly) {
      // Comes back as "YYYY-MM-DD" with no time component; treat it as local midnight.
      const d = /^(\d{4})-(\d\d)-(\d\d)/.exec(String(t));
      if (!d) return null;
      t = zonedToEpoch(+d[1], +d[2], +d[3], 0, 0, 0, src.timezone);
      if (h === undefined) h = 0;
    } else if (src.whenKind === 'wall') { t = arcOut(src, t); if (h === undefined) h = new Date(a[fl.when]).getUTCHours(); }
  ...
  const arcgis = {
    async newest(src) {
      ...
      let m = json.features && json.features[0] && json.features[0].attributes.m;
      if (!m) throw new Error('Could not read the newest record date');
      if (src.fields.whenIsDateOnly) {
        const d = /^(\d{4})-(\d\d)-(\d\d)/.exec(String(m));
        return { asOf: zonedToEpoch(+d[1], +d[2], +d[3], 23, 59, 0, src.timezone) }; // local end-of-day, used only as a moment; date filters re-derive Y-M-D via tzParts
      }
    ...
    async last(src, { lat, lng, radiusM, group, tod, to }) {
      ...
      const params = new URLSearchParams({
        where: src.fields.offense + ' IN (' + arcNames(src, group).map((n) => arcQuote(src, n)).join(',') + ')' + arcTod(src, tod || 'any') +
          ' AND ' + src.fields.when + ' <= ' + arcDateLit(src, end + (src.fields.whenIsDateOnly ? 0 : 60000)) +
          ' AND ' + src.fields.when + ' >= ' + arcDateLit(src, end - LOOKBACK_DAYS * DAY),
  ...
  // Some Socrata datasets store a point as one "location" column instead of separate lat/lon columns. `fields.lat`/`fields.lon`
  // can then be given as 'columnName.latitude' / 'columnName.longitude', a path SoQL accepts directly in $select and $where.
  // Reading the row back only gives the top-level alias we asked for, which socSelect already turns into a plain name.
  const socAlias = (path) => path.replace(/\./g, '_');
  ...
  const socSelect = (src) => {
    const f = src.fields;
    const list = [f.id, src.when.field, src.when.timeField, f.offense, f.detail].concat(f.place || [], f.area || []).filter(Boolean);
    const geo = f.lat.includes('.') ? [f.lat + ' as ' + socAlias(f.lat), f.lon + ' as ' + socAlias(f.lon)] : [f.lat, f.lon];
    return Array.from(new Set(list.concat(geo))).join(',');
  };
  const socOrder = (src) => src.when.field + ' DESC' + (src.when.kind === 'split' ? ', ' + src.when.timeField + ' DESC' : '');

  function socRow(src, r, cache) {
    const f = src.fields;
    const lat = parseFloat(r[f.lat.includes('.') ? socAlias(f.lat) : f.lat]);
    const lng = parseFloat(r[f.lon.includes('.') ? socAlias(f.lon) : f.lon]);
  ...
  const socrata = {
    ...
    async reference(src, { from, to, group, tod }) {
      ...
      const json = await getJson(socUrl(src, soql({
        ...
        // (num() passes lat/lon straight through here; a dotted location path works the same in a round() expression as a plain column)
