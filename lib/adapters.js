  ...
  const arcgis = {
    ...
    async window(src, { bbox, from, to, group, tod, limit }) {
      ...
      for (let offset = 0; ;) {
        ...
        for (const f of feats) { const r = arcRow(f); if (r && keep(src, r.lat, r.lng)) rows.push(r); }
    ...
    async last(src, { lat, lng, radiusM, group, tod, to }) {
      const params = new URLSearchParams({
        where: 'Statute_CrimeCategory IN (' + S.categoriesFor(group).join(',') + ')' + arcTod(tod || 'any') +
          ...
          " AND OccurredFrom_Timestamp >= timestamp '" + sqlTs((to || Date.now()) - LOOKBACK_DAYS * DAY) + "'",
      ...
      const r = f ? arcRow(f) : null;
      return r && keep(src, r.lat, r.lng) ? r : null;
    },
