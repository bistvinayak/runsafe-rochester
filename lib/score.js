...
(function (root) {
  ...
  function filterIncidents(all, opts, now) {
    const cutoff = (now || Date.now()) - opts.days * 86400000;
    const out = [];
    for (const r of all) {
  ...
  function buildReference(scoreIdx, maskIdx, radiusM) {
    ...
    const stepLat = 200 / 111320;
    const midLat = (minLat + maxLat) / 2;
    const stepLng = 200 / (111320 * Math.cos((midLat * Math.PI) / 180));
    const scores = [];
  ...
  const api = {
    CATEGORIES, GridIndex, meters, filterIncidents, resample,
