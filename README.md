...
Chrome extension that shows recent reported crime from police open data as a heatmap on Google Maps, and rates running routes. Covers 16 areas so far: 15 of the 50 largest US cities plus Rochester, NY. See [COVERAGE.md](COVERAGE.md) for the 50 largest US cities.
...
Fifteen of the 50 largest US cities plus Rochester, NY. The full table, with each city's data system, freshness and caveats, is in [COVERAGE.md](COVERAGE.md). Rochester and New York City were set up and checked by hand; the rest were generated by the build tools, passed automated checks against the live data, and are labeled "automatic, not reviewed" in the card.
...
- `lib/score.js`, `lib/format.js`, `lib/sources.js`, `lib/adapters.js` and `lib/live.js` have no dependency on the browser extension APIs. `lib/cities.js` is generated by `tools/gen-cities.js`; do not edit it by hand.

### Adding an area

Use the build tools (they inspect a dataset, map its offenses, find the city's bounds and validate the result against the live server). The steps are at the bottom of [COVERAGE.md](COVERAGE.md). For a data system that has no adapter yet, add one to `lib/adapters.js` that answers `newest`, `window`, `reference`, `last` and `count`. Add the data host to `host_permissions` in `manifest.json` (the background test fails if you forget), and `node test/smoke.js` picks up every configured city automatically.
